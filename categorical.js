...
function createDistribution(items, scoreFunction) {
  ...
  if (!Array.isArray(items)) {
    throw new Error(
      "First argument to `createDistribution` must be an array, and is not."
    );
  }

  if (typeof scoreFunction !== "function") {
    throw new Error(
      "Second argument to `createDistribution` must be a function, and it is " +
        typeof scoreFunction
    );
  }
  ...
  function addItemToDistribution(item) {
    ...
    distro.push({
      probStart: probabilityAccumulator,
      probEnd: probabilityAccumulator + normalizedItemScore,
      item: item
    });
...
function getRandomItem(distribution) {
  ...
  throw new Error(
    "Reached end of distribution without finding anything. Invalid distribution."
  );
}
